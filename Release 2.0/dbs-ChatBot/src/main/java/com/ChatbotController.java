...
public class ChatbotController {
	...
	public BotResponse getBotResponse(@RequestBody BotRequest botRequest){
		BotMessage botRequestMessage= new BotMessage(UUID.randomUUID().toString(),botRequest.getUid(),0,botRequest.getRequestData());
		ChatBotRepo.save(botRequestMessage);
		String responseInfo = technicalChatBot.getBotResponseFor(botRequest.getRequestData());
		BotMessage botResponseMessage= new BotMessage(UUID.randomUUID().toString(),botRequest.getUid(),1,responseInfo);
	...
	public PostResponse likeAnswerMethod(@RequestBody LikeAnswer likeAnswer){
		...
		if(likeAndDislikeRepo.checkEntry(likeAnswer.getUser_id(),likeAnswer.getAnswer_id()).size()==0) {
			likeData.setLike_id(UUID.randomUUID().toString());
			likeData.setAnswer_id(likeAnswer.getAnswer_id());
			likeData.setUser_id(likeAnswer.getUser_id());
			likeData.setType(1);
	...
	public PostResponse dislikeAnswerMethod(@RequestBody LikeAnswer likeAnswer){
		PostResponse postResponse = new PostResponse();
		AnswersData answerData = new AnswersData();
		LikeAndDislike disLikeData = new LikeAndDislike();
		if(likeAndDislikeRepo.checkEntry(likeAnswer.getUser_id(),likeAnswer.getAnswer_id()).size()==0) {
			disLikeData.setLike_id(UUID.randomUUID().toString());
			disLikeData.setAnswer_id(likeAnswer.getAnswer_id());
			disLikeData.setUser_id(likeAnswer.getUser_id());
			disLikeData.setType(0);
			likeAndDislikeRepo.save(disLikeData);
			answerData = AnswerRepo.getOne(likeAnswer.getAnswer_id());
