...
export class TqComponent implements OnInit {
 ...
 postQuestion(ques:any){
   ...
   let uid:any =  this._sessionService.getFromSession("user_id");
   console.log(uid);
   let question:QuestionRequest = new QuestionRequest(ques.value,uid,ques.value);
 ...
 addAnswer(ans:any,question_id:any){
  ...
  let uid:any =  this._sessionService.getFromSession("user_id");
  console.log(uid);
  let answer:AnswerRequest = new AnswerRequest(uid,question_id,ans.value,0,0);
 ...
 likeMethod(answer_id:any){
   ...
   let uid:any =  this._sessionService.getFromSession("user_id");
  console.log(uid);
   let answer:LikeAnswerRequest = new LikeAnswerRequest(answer_id,uid);
